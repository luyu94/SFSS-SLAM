...
{
    //===========
    LOG(INFO) << "SearchByProjection MapPoints";
        ...
        if(bestDist<=TH_HIGH)
        {
            ...
            if(bestLevel != bestLevel2 || bestDist <= mfNNratio * bestDist2){
                if (pMP->GetMovingProbability() >= 0.6) {
                    ...
                    F.mvbOutliers[bestIdx] = true;   //是动态点，mvbKptOutliers值就为true
                    cv::circle(showFeature, uv, 3, cv::Scalar(0, 0, 255), -1);  // 动态点是红色
                    continue;
                }
                if (pMP->GetMovingProbability() > 0.35 && pMP->GetMovingProbability() < 0.5) {
                    initMatchedPoints.insert(std::pair<int, MapPoint*>(bestIdx, pMP));
                    // cv::Point uv = F.mvKeys[bestIdx].pt;
                    // cv::circle(showFeature, uv, 3, cv::Scalar(0, 215, 255), -1);  // cv::Scalar颜色是BGR和透明度
                }
                if (pMP->GetMovingProbability() <= 0.35) {
                    F.mvbOutliers[bestIdx] = false;
                    //F.mvpMapPoints[bestIdx] = pMP;
                    goodMatchedPoints.insert(std::pair<int, MapPoint*>(bestIdx, pMP));
                    cv::Point uv = F.mvKeys[bestIdx].pt;
                    cv::circle(showFeature, uv, 3, cv::Scalar(0, 255, 0), -1);  // 静态点
                }
    ...
    for (auto it = goodMatchedPoints.begin(); it != goodMatchedPoints.end(); it++) {
        ...
        // cv::circle(showFeature, uv, 4, cv::Scalar(0, 255, 0), -1);  // 静态点
    }
    nmatches = nSizeGoodMapPoints;
    LOG(INFO) << "------Num of GoodMapPoints matched: " << nmatches;

    if (nSizeGoodMapPoints < 20) {  //如果去掉动态点之后，SizeGoodMapPoints小于阈值，就加上initMatchedPoints
        for (auto it = initMatchedPoints.begin(); it != initMatchedPoints.end(); it++) {
            MapPoint* pMP = it->second;
            F.mvpMapPoints[it->first] = pMP;
            // cv::Point uv(pMP->mTrackProjX, pMP->mTrackProjY);
            // cv::Point uv = F.mvKeys[it->first].pt;
            // cv::circle(showFeature, uv, 5, cv::Scalar(0, 215, 255), -1);  // cv::Scalar颜色是BGR和透明度
        }
...
{
    //===================
    LOG(INFO) << "SearchByProjection LastFrame";
    ...
    LOG(INFO) << "------mvbOutliers.size: " << LastFrame.mvbOutliers.size() << endl;
    for(int i=0; i<LastFrame.N; i++)
    {
        ...
        {
            // LOG(INFO) << "pMP";
            if(!LastFrame.mvbOutlier[i]  && !LastFrame.mvbOutliers[i])   //
            {
                // Project
                // LOG(INFO) << "------i: " << i << endl;
                ...
                {
                    // LOG(INFO) << "ppp";
                    if (CurrentFrame.mvProbability[i] >= 0.6) {
                        ...
                        // cv::circle(showFeature, uv2, 4, cv::Scalar(0, 0, 255), -1); // 动态点画红色
                        continue;
                    }

                    // CurrentFrame.mvpMapPoints[bestIdx2]=pMP;
                    nmatches++;
                    if (CurrentFrame.mvProbability[i] > 0.35 && CurrentFrame.mvProbability[i] < 0.5) {
                        // LOG(INFO) << "initMatchedPoints";
                        initMatchedPoints.insert(std::pair<int, MapPoint*>(bestIdx2, pMP));
                        // cv::Point uv2 = CurrentFrame.mvKeys[bestIdx2].pt;
                        // cv::circle(showFeature, uv2, 3, cv::Scalar(0, 215, 255), -1); // unknown 黄色
                    }
                    if (CurrentFrame.mvProbability[i] <= 0.35) {
                        // LOG(INFO) << "goodMatchedPoints";
    ...
    for (auto it = goodMatchedPoints.begin(); it != goodMatchedPoints.end(); it++) {
        ...
        cv::circle(showFeature, uv2, 4, cv::Scalar(0, 255, 0), -1);  // 静态点
    }
    nmatches = nSizeGoodMapPoints;
    LOG(INFO) << "******Num of SizeGoodMapPoints matched: " << nmatches;

    if (nSizeGoodMapPoints < 20) {  //只画初始匹配点
        for (auto it = initMatchedPoints.begin(); it != initMatchedPoints.end(); it++) {
            MapPoint* pMP = it->second;
            CurrentFrame.mvpMapPoints[it->first] = pMP;
            cv::Point uv2 = CurrentFrame.mvKeys[it->first].pt;
            cv::circle(showFeature, uv2, 4, cv::Scalar(0, 215, 255), -1); // 增加初始
        }
        nmatches = nSizeGoodMapPoints + nSizeInitMapPoints;
    }
