...
OpticalFlow::OpticalFlow() {
    ...
    LOG(INFO) << "------Importing RAFT Settings...";
    // ----初始化
    Py_Initialize();
    if (!Py_IsInitialized())
    {
    ...
    string dir = "sys.path.append('/mnt/lySLAM/src/RAFT/core')";
...
void OpticalFlow::GetOpticalFlow(const std::vector<cv::Mat>& in_image1, const std::vector<cv::Mat>& in_image2, std::vector<cv::Mat>& optical_img, std::vector<cv::Mat>& optical_uv) {
    // -----循环执行任务
    ...
    int x = 640;
    int y = 480;
    int z = in_image1[0].channels();

    int batch_size = in_image1.size();
    ...
    for (size_t i = 0; i < batch_size; i++)
    {
        ...
        cv::Mat optical = cv::Mat::zeros(480, 640, CV_8UC3); 
        cv::Mat uv = cv::Mat::zeros(480, 640, CV_32FC2);
        // LOG(INFO) << "------CArrays";
        ...
        NumpyAPI::Mat2Py_Array(in_image1[i], CArrays1) ;
        NumpyAPI::Mat2Py_Array(in_image2[i], CArrays2) ;
        ...
        //转换从Python读取的数据
        PyObject *py_optical_uv = PyTuple_GetItem(py_optical, 0);  //PyTuple_GetItem
        PyObject *py_optical_img = PyTuple_GetItem(py_optical, 1);
        if (py_optical_img == nullptr)
        {
            PyErr_Print();
            std::exit(1);
        }
        if (py_optical_uv == nullptr)
        {
            PyErr_Print();
            std::exit(1);
        }
        NumpyAPI::Py_Array2Mat(py_optical_img, optical);
        NumpyAPI::Py_Array2Mat_uv(py_optical_uv, uv);

        optical_img.push_back(optical);
        optical_uv.push_back(uv);

        // LOG(INFO) << "输出";
        // LOG(INFO) << "===========================================";
        // cout << format(optical_uv, Formatter::FMT_PYTHON) << endl << endl;
...
void OpticalFlow::SaveResults(const cv::Mat &in_image1, const cv::Mat &imD1, const cv::Mat &imDepth1, 
    ...
    cv::imwrite("/mnt/SceneFlow1/output/rgb/" + name, in_image1);
    cv::imwrite("/mnt/SceneFlow1/output/Vxyz/" + name, imD1);
    cv::imwrite("/mnt/SceneFlow1/output/depth/" + name, imDepth1);

    cv::imwrite("/mnt/SceneFlow1/output/optical/" + name, optical);

    cv::imwrite("/mnt/SceneFlow1/output/binary_Mask/" + name, binary_Mask);
    cv::imwrite("/mnt/SceneFlow1/output/error_Mask/" + name, error_img);
    cv::imwrite("/mnt/SceneFlow1/output/projection_error/" + name, projection_error);
    cv::imwrite("/mnt/SceneFlow1/output/add/" + name, add);
    cv::imwrite("/mnt/SceneFlow1/output/Floodfill/" + name, bMask);
}