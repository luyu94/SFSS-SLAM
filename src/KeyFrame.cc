...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    , mvbOutliers(F.mvbOutliers)  //
    , mvbOutlier(F.mvbOutlier)         // 
    , mvbKptSceneFlowOutliers(F.mvbSceneFlowOutliers)    //
    , mvbKptSemanticOutliers(F.mvbSemanticOutliers)    //
{
    mImRGB = F.mImRGB.clone();
    mImSemanticMask = F.mImMask.clone();
    // mImSceneMask = F.mImSceneMask.clone();

    mnId=nNextId++;
    LOG(INFO) << "KeyFrame build: " << mnId << "   "<< mnFrameId;

    KeyFrame::KeyFrameUpdatePrioriMovingProbabilitySF();
...
void KeyFrame::KeyFrameUpdatePrioriMovingProbabilitySF()
{
    cv::Mat showFeature = this->mImRGB.clone();
    cv::Mat showProbability = this->mImRGB.clone();
    ...
    for (int i = 0; i < this->N; i++) {
        ...
        if (kp.pt.x <= 0 || kp.pt.x >= this->mImSemanticMask.cols)
            continue;
        if (kp.pt.y <= 0 || kp.pt.x >= this->mImSemanticMask.rows)
            continue;
        ...
        //mask二值图， 255白色
        // if (this->mImSceneFlowMask.at<uchar>((int)kp.pt.y, (int)kp.pt.x) == 255 || this->mImSemanticMask.at<uchar>((int)kp.pt.y, (int)kp.pt.x) == 255) {  //  || this->mImSemanticMask.at<uchar>((int)kp.pt.y, (int)kp.pt.x) == 255
        //     this->mnDynamicPoints++;
        // }

        // if (this->mImSceneFlowMask.at<uchar>((int)kp.pt.y, (int)kp.pt.x) == 255) {
            
        //     // dynamic object exists
        //     // visualization
        //     // cv::circle(showFeature, kp.pt, 3, cv::Scalar(0, 0, 255), -1); //场景流检测的动态点是红色
        //     this->mvbKptSceneFlowOutliers[i] = true;

        // } else {
        //     // visualization
        //     // cv::circle(showFeature, kp.pt, 3, cv::Scalar(0, 255, 0), -1);  //静态点 绿色
        //     this->mvbKptSceneFlowOutliers[i] = false;

        // }
        
        if (this->mImSemanticMask.at<uchar>((int)kp.pt.y, (int)kp.pt.x) == 255) {
            
            this->mvbKptSemanticOutliers[i] = true;

        } else {
            // visualization
            // cv::circle(showFeature, kp.pt, 3, cv::Scalar(0, 255, 0), -1);  //静态点 绿色
            this->mvbKptSemanticOutliers[i] = false;
        ...
        if (bIsMapPointExists) {   //+++++++++++++++贝叶斯   地图点存在就保存概率图
            ...
            if (this->mvbKptSemanticOutliers[i] && this->mvbKptSceneFlowOutliers[i]) {   //是两种动态点
                float p_d = p_zd_md * p_old_d;
                float p_s = p_zd_ms * p_old_s;
                float eta = 1 / (p_d + p_s);
                pMP->SetMovingProbability(eta * p_d);

            } else if (!this->mvbKptSemanticOutliers[i] && !this->mvbKptSceneFlowOutliers[i]){ //两种都不是
                float p_d = p_zs_md * p_old_d;
                float p_s = p_zs_ms * p_old_s;
                float eta = 1 / (p_d + p_s);
                pMP->SetMovingProbability(eta * p_d);

            } else if (this->mvbKptSemanticOutliers[i] && !this->mvbKptSceneFlowOutliers[i] ) {   //只是Semantic动态点
                float p_d1 = p_zd_md1 * p_old_d;
                float p_s1 = p_zd_ms1 * p_old_s;
                float eta1 = 1 / (p_d1 + p_s1);
                pMP->SetMovingProbability(eta1 * p_d1);

            } else if (this->mvbKptSceneFlowOutliers[i] && !this->mvbKptSemanticOutliers[i]) {   //是SceneFlow动态点
            ...
            // if (this->mvbKptSemanticOutliers[i]) {   //只是Semantic动态点
            //     float p_d1 = p_zd_md1 * p_old_d;
            //     float p_s1 = p_zd_ms1 * p_old_s;
            //     float eta1 = 1 / (p_d1 + p_s1);
            //     pMP->SetMovingProbability(eta1 * p_d1);
            ...
            // if (this->mvbKptSceneFlowOutliers[i]) {   //只是Scene动态点
            //     float p_d2 = p_zd_md2 * p_old_d;
            //     float p_s2 = p_zd_ms2 * p_old_s;
            //     float eta2 = 1 / (p_d2 + p_s2);
            //     pMP->SetMovingProbability(eta2 * p_d2);
    ...
    Config::GetInstance()->saveImage(showProbability, "Probability_SceneFlow", "KeyFrame_Probability_" + std::to_string(this->mnId) + "_" + std::to_string(this->mnFrameId) + ".png");
}
